...
export function usePriceHistory(params?: PriceHistoryParams) {
  ...
  const queryParams = {
    ...
    days,
    fishTypeId:
      selectedFishTypes.length === 1 ? selectedFishTypes[0] : undefined,
  };
  ...
  return useQuery({
    ...
    select: (data) => {
      ...
      predictions = predictions.filter(
        (p) => parseFloat(p.confidence) >= confidenceThreshold
      );
      ...
      if (!fishTypeId && selectedFishTypes.length > 1) {
        predictions = predictions.filter((p) =>
          selectedFishTypes.includes(p.fishTypeId)
        );
      }
...
export function useAddActualPrice() {
  ...
  return useMutation({
    mutationFn: (priceData: AddActualPriceRequest) =>
      apiClient.addActualPrice(priceData),
    onSuccess: (data) => {
    ...
    onError: (error) => {
      ...
      addNotification({
        ...
        message:
          "Failed to add actual price. Please check your data and try again.",
      });
    },
  });
}
...
function calculatePriceStats(prices: any[]) {
  ...
  const avgRetail =
    retailPrices.reduce((sum, price) => sum + price, 0) / retailPrices.length;
  const avgWholesale =
    wholesalePrices.reduce((sum, price) => sum + price, 0) /
  ...
  const retailStdDev = Math.sqrt(
    retailPrices.reduce(
      (sum, price) => sum + Math.pow(price - retailMean, 2),
      0
    ) / retailPrices.length
  );
...
function calculateAverageConfidence(predictions: any[]) {
  ...
  const totalConfidence = predictions.reduce(
    (sum, p) => sum + parseFloat(p.confidence),
    0
  );
...
function calculateTrends(prices: any[]) {
  ...
  return Object.entries(pricesByFish).map(([fishTypeId, fishPrices]) => {
    ...
    const sortedPrices = typedFishPrices.sort(
      (a, b) =>
    ...
    return {
      ...
      latestPrice: parseFloat(
        sortedPrices[sortedPrices.length - 1]?.retailPrice || "0"
      ),
